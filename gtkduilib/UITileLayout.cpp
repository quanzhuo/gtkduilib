#include "stdafx.h"
#include "UITileLayout.h"

CUITileLayout::CUITileLayout() : m_nColumns(1)
{
	m_szItem.cx = m_szItem.cy = 0;
}

CUIString CUITileLayout::GetClass() const
{
	return L"TileLayoutUI";
}

LPVOID CUITileLayout::GetInterface(const CUIString& strName)
{
	if(strName == DUI_CTR_TILELAYOUT) 
		return static_cast<CUITileLayout*>(this);
	return 
        CUIContainer::GetInterface(strName);
}

SIZE CUITileLayout::GetItemSize() const
{
	return m_szItem;
}

void CUITileLayout::SetItemSize(SIZE szItem)
{
	if(m_szItem.cx != szItem.cx || m_szItem.cy != szItem.cy){
		m_szItem = szItem;
		NeedUpdate();
	}
}

int CUITileLayout::GetColumns() const
{
	return m_nColumns;
}

void CUITileLayout::SetColumns(int nCols)
{
	if(nCols <= 0) 
		return;
	m_nColumns = nCols;
	NeedUpdate();
}

void CUITileLayout::SetAttribute(LPCWSTR pstrName, LPCWSTR pstrValue)
{
	if(wcscmp(pstrName, L"itemsize") == 0){
		CUISize szItem;
		LPWSTR pstr = NULL;
		szItem.cx = wcstol(pstrValue, &pstr, 10);  //ASSERT(pstr);
		szItem.cy = wcstol(pstr + 1, &pstr, 10);   //ASSERT(pstr);
		SetItemSize(szItem);
	}else if(wcscmp(pstrName, L"columns") == 0)
		SetColumns(/*_wtoi(pstrValue)*/ wcstol(pstrValue, NULL, 10));
	else
		CUIContainer::SetAttribute(pstrName, pstrValue);
}

void CUITileLayout::SetPos(const CUIRect& rc)
{
    CUIRect rcTemp = rc;
	CUIControl::SetPos(rc);
	rcTemp = m_rcItem;

	//
	// 调整内边距
	//

	rcTemp.left += m_rcInset.left;
	rcTemp.top += m_rcInset.top;
	rcTemp.right -= m_rcInset.right;
	rcTemp.bottom -= m_rcInset.bottom;
	
	//
	// 没有子控件不处理
	//
	
	if( m_items.GetSize() == 0) {
		ProcessScrollBar(rcTemp, 0, 0);
		return;
	}
	
	//
	// 减去滚动条的长宽
	//
	
	if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) 
		rcTemp.right -= m_pVerticalScrollBar->GetFixedWidth();
	if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) 
		rcTemp.bottom -= m_pHorizontalScrollBar->GetFixedHeight();

	//
	// Position the elements
	// 计算列数
	//

	if(m_szItem.cx > 0) 
		m_nColumns = rcTemp.GetWidth() / m_szItem.cx;
	if(m_nColumns == 0) 
		m_nColumns = 1;

	int cyNeeded = 0;
	int cxWidth;
	
	//
	// 计算每列的宽度
	//
	
	if(m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()) 
		cxWidth = (rcTemp.GetWidth() + m_pHorizontalScrollBar->GetScrollRange() ) / m_nColumns; 
	else
		cxWidth = (rcTemp.GetWidth()) / m_nColumns;

	int cyHeight = 0;
	int iCount = 0;
	POINT ptTile = { rcTemp.left, rcTemp.top };
	if(m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()){
		ptTile.y -= m_pVerticalScrollBar->GetScrollPos();
	}

	int iPosX = rcTemp.left;
	if(m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()){
		iPosX -= m_pHorizontalScrollBar->GetScrollPos();
		ptTile.x = iPosX;
	}
	for( int it1 = 0; it1 < m_items.GetSize(); it1++ ){
		CUIControl* pControl = static_cast<CUIControl*>(m_items[it1]);
		if(!pControl->IsVisible()) 
			continue;

		if(pControl->IsFloat()){
			SetFloatPos(it1);
			continue;
		}

		//
		// Determine size
		//

		RECT rcTile = {ptTile.x, ptTile.y, ptTile.x + cxWidth, ptTile.y};
		if( (iCount % m_nColumns) == 0 ){
			int iIndex = iCount;
			
			//
			// 取本行最大的长度
			//
			
			for( int it2 = it1; it2 < m_items.GetSize(); it2++ ){
				CUIControl* pLineControl = static_cast<CUIControl*>(m_items[it2]);
				if( !pLineControl->IsVisible() ) 
					continue;
				if( pLineControl->IsFloat() ) 
					continue;

				RECT rcPadding = pLineControl->GetPadding();
				SIZE szAvailable = { rcTile.right - rcTile.left - rcPadding.left - rcPadding.right, 9999 };
				
				//
				// 第一个和最后一个只减一次padding
				//
				
				if(iIndex == iCount || (iIndex + 1) % m_nColumns == 0){
					szAvailable.cx -= m_iChildPadding / 2;
				}else{
					szAvailable.cx -= m_iChildPadding;
				}

				//
				// 不能超过最大值和最小值
				//

				if( szAvailable.cx < pControl->GetMinWidth()) 
					szAvailable.cx = pControl->GetMinWidth();
				if( szAvailable.cx > pControl->GetMaxWidth()) 
					szAvailable.cx = pControl->GetMaxWidth();
				
				SIZE szTile = pLineControl->EstimateSize(szAvailable);
				if( szTile.cx < pControl->GetMinWidth()) 
					szTile.cx = pControl->GetMinWidth();
				if( szTile.cx > pControl->GetMaxWidth()) 
					szTile.cx = pControl->GetMaxWidth();
				if( szTile.cy < pControl->GetMinHeight()) 
					szTile.cy = pControl->GetMinHeight();
				if( szTile.cy > pControl->GetMaxHeight()) 
					szTile.cy = pControl->GetMaxHeight();

				cyHeight = MAX(cyHeight, szTile.cy + rcPadding.top + rcPadding.bottom);
				if( (++iIndex % m_nColumns) == 0) 
					break;
			}
		}

		RECT rcPadding = pControl->GetPadding();
		
		//
		// 第一个和最后一个特殊处理一下
		//
		
		rcTile.left += rcPadding.left + m_iChildPadding / 2;
		rcTile.right -= rcPadding.right + m_iChildPadding / 2;
		if( (iCount % m_nColumns) == 0 ) {
			rcTile.left -= m_iChildPadding / 2;
		}

		if( ( (iCount + 1) % m_nColumns) == 0 ) {
			rcTile.right += m_iChildPadding / 2;
		}

		//
		// Set position
		// 将长度设置为本行最大的长度
		//

		rcTile.top = ptTile.y + rcPadding.top;
		rcTile.bottom = ptTile.y + cyHeight;

		SIZE szAvailable = { rcTile.right - rcTile.left, rcTile.bottom - rcTile.top };
		SIZE szTile = pControl->EstimateSize(szAvailable);
		if(szTile.cx == 0 ) szTile.cx = szAvailable.cx;
		if(szTile.cy == 0 ) szTile.cy = szAvailable.cy;
		if( szTile.cx < pControl->GetMinWidth()) 
			szTile.cx = pControl->GetMinWidth();
		if( szTile.cx > pControl->GetMaxWidth()) 
			szTile.cx = pControl->GetMaxWidth();
		if( szTile.cy < pControl->GetMinHeight()) 
			szTile.cy = pControl->GetMinHeight();
		if( szTile.cy > pControl->GetMaxHeight()) 
			szTile.cy = pControl->GetMaxHeight();
		
		//
		// 生成新的Pos,并设置
		//
		
		RECT rcPos = {(rcTile.left + rcTile.right - szTile.cx) / 2, (rcTile.top + rcTile.bottom - szTile.cy) / 2,
			(rcTile.left + rcTile.right - szTile.cx) / 2 + szTile.cx, (rcTile.top + rcTile.bottom - szTile.cy) / 2 + szTile.cy};
		pControl->SetPos(rcPos);

		if( (++iCount % m_nColumns) == 0 ) {
			ptTile.x = iPosX;
			ptTile.y += cyHeight + m_iChildPadding;
			cyHeight = 0;
		}else{
			ptTile.x += cxWidth;
		}
		cyNeeded = rcTile.bottom - rcTemp.top;
		if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible() ) 
			cyNeeded += m_pVerticalScrollBar->GetScrollPos();
	}

	//
	// Process the scrollbar
	//

	ProcessScrollBar(rcTemp, 0, cyNeeded);
}
